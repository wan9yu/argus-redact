...
def test_ssn_should_be_redacted_when_followed_by_arithmetic_suffix():
    out, _ = redact("SSN 123-45-6789 - 5 dollars", lang="en", mode="fast", salt=42)
    assert "123-45-6789" not in out


def test_credit_card_should_be_redacted_when_followed_by_arithmetic_suffix():
    out, _ = redact("card 4111111111111111 - 0", lang="en", mode="fast", salt=42)
    assert "4111111111111111" not in out


def test_ip_address_should_not_be_redacted_when_text_is_a_version_string():
...
def test_checksum_valid_id_should_be_redacted_when_preceded_by_false_positive_prefix():
...
def test_same_type_values_should_both_be_redacted_when_split_by_slash():
...
def test_same_type_values_should_both_be_redacted_when_split_by_spaced_delimiter():
...
def test_ip_range_endpoints_should_both_be_redacted_when_joined_by_a_hyphen():
...
def test_phone_without_validator_should_be_suppressed_when_followed_by_arithmetic_operand():
...
def test_phone_should_be_suppressed_when_preceded_by_false_positive_prefix():
...
def test_itin_should_be_redacted_without_affecting_ssn_detection():

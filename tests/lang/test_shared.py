...
class TestIPAddress:
    ...
    def test_should_match_or_reject_when_ip_input(self, shared_patterns, example):
        ...
        if example["should_match"]:
            assert len(ip_results) >= 1, f"Expected match: {example['description']}"
            if "expected_text" in example:
                assert any(r.text == example["expected_text"] for r in ip_results)
        else:
            assert len(ip_results) == 0, f"Should NOT match: {example['description']}"

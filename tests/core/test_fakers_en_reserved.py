...
class TestFakePhoneEnReserved:
    def test_should_use_555_01xx_format(self):
        result = fake_phone_en_reserved("(415) 555-1234", random.Random(1))
        # Format: (555) 555-01XX
        assert re.match(r"^\(555\) 555-01\d{2}$", result), f"Got {result}"

    def test_should_be_deterministic(self):
        a = fake_phone_en_reserved("orig", random.Random(7))
        b = fake_phone_en_reserved("orig", random.Random(7))
        assert a == b


class TestFakeSsnEnReserved:
    def test_should_use_999_area(self):
        result = fake_ssn_en_reserved("123-45-6789", random.Random(1))
        assert re.match(r"^999-\d{2}-\d{4}$", result), f"Got {result}"

    def test_should_be_deterministic(self):
        a = fake_ssn_en_reserved("orig", random.Random(7))
        b = fake_ssn_en_reserved("orig", random.Random(7))
        assert a == b
...
class TestFakePersonEnReserved:
    def test_should_use_canonical_fake_name(self):
        result = fake_person_en_reserved("John Smith", random.Random(1))
...
class TestFakeAddressEnReserved:
    def test_should_use_fictional_address(self):
        result = fake_address_en_reserved("1234 Main St, Anytown, USA", random.Random(1))

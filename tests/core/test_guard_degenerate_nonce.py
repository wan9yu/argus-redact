...
let three degenerate nonces through, and the nonce stripper then destroyed or
corrupted the caller's text while the call reported a clean COMPLETE restore:

- an EMPTY nonce is a substring of everything, so provenance passed and
  stripping "" sliced the whole text to "" (data destruction);
- a nonce that is an incidental substring of the text (a common character like
  "的") passed provenance, and every occurrence of it was removed;
...
Every test here drives the public `restore(guard=True, anchor=...)` path, so it
asserts the live enforcement (which lives in the Rust core) rather than any
shim-side copy of it.
"""

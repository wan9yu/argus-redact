...
(mask/name_mask/landline_mask/category) lose information by design
...
class TestStrategyClassification:
    ...
    def test_is_strategy_reversible_should_be_true_for_reversible_strategies(self, strategy):
        assert is_strategy_reversible(strategy) is True

    @pytest.mark.parametrize("strategy", ["mask", "name_mask", "landline_mask", "category"])
    def test_is_strategy_reversible_should_be_false_for_irreversible_strategies(self, strategy):
        assert is_strategy_reversible(strategy) is False

    def test_is_strategy_reversible_should_classify_every_valid_strategy(self):
...
class TestUnknownStrategy:
    def test_is_strategy_reversible_should_raise_value_error_when_strategy_is_unknown(self):
...
class TestRoundTripBehavior:
    ...
    def test_restore_should_return_original_when_strategy_is_pseudonym(self):
        entities = [_match("张三", "person", 0)]
        redacted, key, _ = replace("张三说了话", entities, salt=42)
        assert restore(redacted, key, guard=False) == "张三说了话"

    def test_restore_should_return_original_when_strategy_is_remove(self):
    ...
    def test_mask_strategy_should_emit_partial_visible_form(self):
        ...
        redacted, _key, _ = replace("13812345678", entities, salt=42)
        assert "****" in redacted


class TestPIITypeDefProperty:
    def test_is_reversible_should_be_true_when_strategy_is_pseudonym(self):
        td = PIITypeDef(name="x", lang="zh", format="", strategy="pseudonym")
        assert td.is_reversible is True

    def test_is_reversible_should_be_false_when_strategy_is_mask(self):
        td = PIITypeDef(name="x", lang="zh", format="", strategy="mask")
        assert td.is_reversible is False

    def test_is_reversible_should_be_true_when_strategy_is_keep(self):
        td = PIITypeDef(name="x", lang="zh", format="", strategy="keep")
        assert td.is_reversible is True


class TestPublicExport:
    def test_is_strategy_reversible_should_be_exported_from_top_level_package(self):
        from argus_redact import is_strategy_reversible as exported

        assert exported is is_strategy_reversible

    def test_is_strategy_reversible_should_be_listed_in_dunder_all(self):
        import argus_redact

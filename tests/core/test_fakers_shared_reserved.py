...
class TestFakeEmailReserved:
    def test_should_use_example_dot_tld(self):
        result, _ = fake_email_reserved("alice@company.com", random.Random(1))
        assert result.endswith(("@example.com", "@example.org", "@example.net")), result

    def test_should_be_deterministic(self):
        a, _ = fake_email_reserved("orig", random.Random(7))
        b, _ = fake_email_reserved("orig", random.Random(7))
        assert a == b


class TestFakeIpReserved:
    def test_should_use_rfc5737_for_ipv4_input(self):
        result, _ = fake_ip_reserved("10.0.0.5", random.Random(1))
    ...
    def test_should_use_rfc3849_for_ipv6_input(self):
        result, _ = fake_ip_reserved("fe80::1", random.Random(1))
        assert result.startswith("2001:db8:"), f"Expected 2001:db8 prefix, got {result}"

    def test_should_be_deterministic(self):
        a, _ = fake_ip_reserved("10.0.0.5", random.Random(3))
        b, _ = fake_ip_reserved("10.0.0.5", random.Random(3))
        assert a == b


class TestFakeMacReserved:
    def test_should_use_rfc7042_documentation_block(self):
        result, _ = fake_mac_reserved("aa:bb:cc:dd:ee:ff", random.Random(1))
        assert re.match(r"^00:00:5E:00:53:[0-9A-Fa-f]{2}$", result), result

    def test_should_be_deterministic(self):
        a, _ = fake_mac_reserved("orig", random.Random(2))
        b, _ = fake_mac_reserved("orig", random.Random(2))
        assert a == b

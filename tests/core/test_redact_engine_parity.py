...
CASES = [
    ...
    ("zh_realistic", "张三的电话13812345678，身份证110101199003074610",
        "zh", {"person": {"strategy": "realistic"}, "phone": {"strategy": "realistic"},
               "id_number": {"strategy": "realistic"}}, None),
    ("zh_mask", "电话13812345678 银行卡6217000000000000", "zh",
        {"phone": {"strategy": "mask"}, "bank_card": {"strategy": "mask"}}, None),
    ("zh_landline_mask", "座机 010-12345678", "zh",
        {"phone_landline": {"strategy": "landline_mask"}, "phone": {"strategy": "landline_mask"}}, None),
    ...
    ("en_realistic", "John Smith SSN 123-45-6789 card 4111111111111111", "en",
        {"person": {"strategy": "realistic"}, "ssn": {"strategy": "realistic"},
         "credit_card": {"strategy": "realistic"}}, None),
    ("en_address", "lives at 1600 Pennsylvania Ave", "en",
        {"address": {"strategy": "realistic"}}, None),
...
def _build():
    ...
    r, k = redact("张三 13812345678 110101199003074610", mode="fast", lang="zh",
                  salt=SALT, unified_prefix="R")
...
def test_redact_engine_parity():
    ...
    if not FIXTURE.exists():
        ...
        raise AssertionError(
            "Wrote v0.7.2 redact-engine snapshot — re-run to compare. COMMIT it."
        )

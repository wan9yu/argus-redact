...
1. **生产部署推荐 32b**：71% 检测率 + 合理的保守度，5.6s 延迟在异步场景可接受
2. **3b 仅适合做快速预筛**：35% 检测率太低，只能捕捉明显的医疗/金融
3. **Prompt 优化空间大**：通过领域知识注入（宗教日历、性别推断规则）可提升 32b 到 80%+
4. **极端隐含场景**（"出来后"→犯罪）可能需要多轮推理或更大模型

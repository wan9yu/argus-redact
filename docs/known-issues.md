...
[`CHANGELOG.md`](../CHANGELOG.md), updated with each release, and is not duplicated
here.

### Earlier (pre-v0.6.6, before `CHANGELOG.md` began)
...
| MCP `_TOKEN_STORE` was an unbounded module-level dict with no eviction | v0.6.2 | Switched to `OrderedDict` with 5-min idle TTL + 100-entry LRU cap. Token access bumps timestamp (sliding window). Per-session binding deferred to v0.7 (MCPServer API survey). |

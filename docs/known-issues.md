...
| **en/person realistic requires `mode="ner"` (or `names=[...]`)** | English person detection is NER-only (spaCy `en_core_web_sm`); there is no fast-mode regex or surname-list fallback like zh. In `mode="fast"`, English names are not detected at all, so realistic replacement (John Doe / Jane Roe) cannot fire. | Run `mode="ner"` to enable spaCy-based person detection, or pass `names=["John Smith", ...]` for fast-mode list-match. |
...
| **Pollution scanner false-positives on canonical fake names** | Common Chinese placeholder names (`张三`, `李四`, `王五`, `赵六`, `钱七`) are also real Chinese names. The reserved-range scanner flags these as already-redacted output, so a real user named 张三 cannot be redacted via `pseudonym-llm`. | Use `strict_input=False` for known-clean inputs; or filter inputs before redaction; or rely on `mode="ner"` + names list for explicit overrides. |

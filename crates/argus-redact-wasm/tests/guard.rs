...
fn provenance_failed_when_nonce_absent_blocks_and_returns_input_unchanged() {
    ...
    let out = restore_guarded(text, key, anchor).expect("a blocked outcome is not an error");
...
fn full_scope_roundtrip_recovers_original_and_strips_nonce() {
    ...
    let out = restore_guarded(&reply, key, anchor).expect("restore_guarded should succeed");

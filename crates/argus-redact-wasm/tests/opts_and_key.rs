...
fn key_survives_json_roundtrip_one_shot() {
    ...
    let redacted: String = Reflect::get(&out, &JsValue::from_str("text"))
        ...
        .as_string()
        .unwrap();
    let restored = restore(&redacted, key_reparsed, JsValue::UNDEFINED).expect("restore should succeed");
    assert_eq!(
        restored, original,
...
fn key_survives_json_roundtrip_streaming() {
    ...
    let key_reparsed = json_roundtrip(&key);
    let restored = restore(&downstream, key_reparsed, JsValue::UNDEFINED).expect("restore should succeed");
    assert_eq!(
        restored, original,

...
[![PyPI](https://img.shields.io/pypi/v/argus-redact)](https://pypi.org/project/argus-redact/) [![crates.io](https://img.shields.io/badge/crates.io-v0.8.15-orange)](https://crates.io/crates/argus-redact-core) [![Tests](https://github.com/wan9yu/argus-redact/actions/workflows/test.yml/badge.svg)](https://github.com/wan9yu/argus-redact/actions/workflows/test.yml) [![codecov](https://codecov.io/gh/wan9yu/argus-redact/graph/badge.svg)](https://codecov.io/gh/wan9yu/argus-redact) [![Demo](https://img.shields.io/badge/🤗-Demo-yellow)](https://huggingface.co/spaces/wan9yu/argus-redact)
...
**78 类 PII，覆盖 3 层**（完整清单见[类型目录](docs/pii-types.md)，由注册表自动生成）— 从电话号码到医疗诊断、宗教信仰、政治立场。默认 `mode="fast"`（仅 L1，零依赖，亚毫秒）；可选 `mode="ner"`（+ NER 模型）→ `mode="auto"`（全部 3 层）。
...
| 人名 | HanLP | spaCy | spaCy | spaCy | spaCy | spaCy | spaCy | — |
| 邮箱 | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
...
默认 redact 输出伪代号与掩码（`P-83811`、`138****5678`），审计清晰且可逆，但真实结构已丢失，下游 LLM 可能难以推理。`pseudonym-llm` profile 把 PII 换成**看起来真实但属于保留段**的假值（如 `19999...` 手机、`999...` 身份证、`999999...` 银行卡）。LLM 推理正常，懂规则的人也能一眼看出是合成的。
